...
class CheckersPiece
{
    ...
    static CheckersPiece[][]chessPieces = new CheckersPiece[colorType.length][pieceType.length];
    
    static void readInImages()
    {
    	for (int i=0; i < colorType.length; i++)
        	for (int j=0; j < pieceType.length; j++)
        		chessPieces[i][j] = new CheckersPiece(i,j);
    }
    ...
    CheckersPiece(int colorIndex, int pieceIndex)
    {
    	String imageName = "images/" +colorType[colorIndex].toString() +
    			pieceType[pieceIndex].toString()+".png";
