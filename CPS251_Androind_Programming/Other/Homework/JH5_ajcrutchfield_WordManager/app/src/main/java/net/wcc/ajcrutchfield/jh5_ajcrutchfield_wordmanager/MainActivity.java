package net.wcc.ajcrutchfield.jh5_ajcrutchfield_wordmanager;
...
public class MainActivity extends Activity implements AdapterView.OnItemClickListener, FileOperations {
    ...
    private static final int DOWLOAD_MANAGER_INTENT_CALL = 4;
    private static final String DOWNLOAD_MANAGER = "net.wcc.ajcrutchfield.intent.action.DOWNLOAD_MANAGER";
    private static final String DOWNLOAD_NAME_LIST = "myNameList";
    ...
    public boolean onOptionsItemSelected(MenuItem item)
    {
        switch (item.getItemId()) {
            ...
            case R.id.download_list:
                downloadListManager();
                Log.d("Mine", "download_list clicked");
                return true;
    ...
    private void downloadListManager() {
        intent = new Intent(DOWNLOAD_MANAGER);
        startActivityForResult(intent, DOWLOAD_MANAGER_INTENT_CALL);
    }
    ...
    //default filename is wrong. Might be from this method
...
//            arr = new ArrayList<String>(); // Empty list
    ...
    //TODO
    public void returnCategory(View view) {
        Log.d("Mine", "sending outData");
        Intent outData = new Intent();
        outData.putStringArrayListExtra(MY_WORD_LIST, myWordListArray);
        outData.putExtra(CATEGORY_NAME, currentFileName);
    ...
    public void onActivityResult(int reqCode, int resCode, Intent data){
        ...
        if(reqCode == DOWLOAD_MANAGER_INTENT_CALL){
            //get array list to pass in
            list = data.getStringArrayListExtra(DOWNLOAD_NAME_LIST);
            Log.d("Mine", list.toString());
            updateFromArrayList(list);
        }
    }
}